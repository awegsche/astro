...
inline auto detect_stars(Frame<RGBFloat> &frame) {
    ...
    for (int i = 10; i < reduced_frame.width() - 10; ++i) {
        for (int j = 10; j < reduced_frame.height() - 10; ++j) {
            ...
            if (reduced_frame.at(i, j) > threshold) {
                ...
                const auto accumulator = [](pixel_acc const &a, pixel_value const &b) -> pixel_acc {
                    return {a.x + (float)b.x * b.value, a.y + (float)b.y * b.value, a.value + b.value};
                };
                float l     = static_cast<float>(adjacent_pixels.size());
                pixel_acc p = std::accumulate(adjacent_pixels.begin(), adjacent_pixels.end(),
                                              pixel_acc{0.0f, 0.0f, 0.0f}, accumulator);
                p.x /= p.value;
                p.y /= p.value;
                stars.push_back({static_cast<int>(p.x), static_cast<int>(p.y), p.value});
            }
        }
    }
